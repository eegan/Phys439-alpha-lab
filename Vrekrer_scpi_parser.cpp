...
SCPI_Commands::SCPI_Commands(char *message) {
  ...
  while (token != NULL) {
    this->Append(token);
    //Dbg.print("Appending "); Dbg.println(token);  //EE
    token = strtok(NULL, ":");
  }
}
...
uint32_t SCPI_Parser::GetCommandCode(SCPI_Commands& commands) {
  ...
  for (uint8_t i = 0; i < commands.Size(); i++) {
    //Dbg.print("Command: "); Dbg.println(i); //EE
    ...
    //Dbg.println(isQuery? "Query" : "Not query"); //EE
    bool isToken;
    for (uint8_t j = 0; j < tokens_size_; j++) {
      ...
      if ( (tokens_[j][long_length - 1] == '#') //Numeric suffix capable token
        ...
        //Dbg.println("Removed numeric suffix"); //EE
      }
    ...
    //if (!isToken) Dbg.println("No code"); //EE
    if (!isToken) return 0;
  }
  if (isQuery) code ^= 0x80000000;
  //Dbg.print("GetCommandCode: "); Dbg.println(code+1); //EE
  return code+1;
}
...
void SCPI_Parser::Execute(char* message, Stream &interface) {
  ...
  //Dbg.print("Execute Code: "); Dbg.println(code);
  for (uint8_t i = 0; i < codes_size_; i++) {
    if (valid_codes_[i] == code) {
      //Dbg.println("Match"); //EE
